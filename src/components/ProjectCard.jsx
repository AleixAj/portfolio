...
const ProjectCard = memo(function ProjectCard({ index = 0, title, img, imgCls, desc, tags, github, demo }) {
  return (
    <div style={{ animationDelay: `${index * 100}ms` }} className="reveal-item group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-cyan-400/50 hover:shadow-[0_0_30px_rgba(34,211,238,0.12)] transition-all duration-300 flex flex-row md:flex-col">
      ...
      <div className="w-1/3 flex-shrink-0 md:w-auto md:h-36 lg:h-40 bg-black/40 flex items-center justify-center overflow-hidden self-stretch">
        <img src={img} alt={title} className={`w-full h-full object-contain md:h-24 lg:h-28 md:w-auto group-hover:scale-105 transition-transform duration-300 ${imgCls ?? 'p-3 md:p-0'}`} />
      </div>

      <div className="flex-1 flex flex-col p-3 md:p-4 min-w-0">
        <h3 className="text-lg md:text-xl font-bold mb-1 md:mb-2 text-white text-center md:text-left">{title}</h3>
        <p className="text-cyan-400/80 text-xs leading-relaxed text-justify">{desc}</p>
        <div className="flex mt-1.5 md:mt-3 gap-1.5 flex-wrap">
          {tags.map(({ label, cls }) => (
            <span key={label} className={`text-xs px-2 py-0.5 md:py-1 rounded-full border ${cls}`}>{label}</span>
          ))}
        </div>
        <div className="mt-auto pt-2 md:pt-4 flex gap-1.5 md:gap-2">
